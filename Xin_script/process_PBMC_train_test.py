...
def process_batch2_ind(data_dir, result_dir, input1, input2, 
        celltype_gran=1):
    ''' Process individual data of PBMC batch2

    @data_dir: where PBMC batch2 data stroes
    @result_dir: where to store PCA/tSNE/UMAP result
    @input1/input2: control/stimulated_ID
    @celltype_gran: granularity of cell types, 0: major cell types, 1:sub-celltypes
        According to the dataset, the give cell types are sub-cell types
    '''
    ## add batch info
    input1_list = input1.split('_')
    train_adata = load_PBMC_data.load_PBMC_batch2_data(data_dir, condition=input1_list[0],
            ind='_'.join(input1_list[1:]))
    input2_list = input2.split('_')
    test_adata = load_PBMC_data.load_PBMC_batch2_data(data_dir, condition=input2_list[0],
            ind='_'.join(input2_list[1:]))
